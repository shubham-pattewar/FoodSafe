...
import { Leaf, LogOut, User, ScanLine, LayoutDashboard, ArrowRightLeft, Menu, X } from 'lucide-react';
      ...
      <AnimatePresence>
        {isMobileMenuOpen && user && (
          <motion.div
            ...
            className="fixed top-24 inset-x-0 mx-auto w-[95%] max-w-sm glass-panel rounded-2xl p-4 flex flex-col space-y-2 z-40 md:hidden"
          >
            {navLinks.map((link) => {
              const isActive = location.pathname === link.path;
              const Icon = link.icon;
              return (
                <Link
                  ...
                  className={`flex items-center space-x-3 px-4 py-3 rounded-xl transition-all ${
                    isActive 
                      ? 'bg-tech/20 text-white border border-tech/30' 
